...
void drawResult(){
	...
	for (int i=1; i<pathSequence.size(); i++){
		...
		drawLine(graph.getNodePosition(), sourceIdx, targetIdx, Vec3(1.0f,0.0f,1.0f), radius, 3.0f, true);
	}
	
	// gambar teks
	sprintf(markText,"%s","start");
	Vec3 position(graph.getNodePosition()[pathSequence.at(0)].getX()+2.0f*radius, graph.getNodePosition()[pathSequence.at(0)].getY()+2.0f*radius, 0.0f);
...
void BBRecursive(int rootIdx, int startIdx, int levelIdx, Graph graph, bool visited[], 
vector<int> path, vector<float> costMatrix[], float costRoot){
	// inisialisasi
